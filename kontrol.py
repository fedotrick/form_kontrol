...
class ReportGenerator:
    ...
    def generate_daily_report(self, date_str, summary_report_file=None):
        ...
        try:
            ...
            # Проверяем, что дата имеет правильный формат (DD.MM.YYYY)
            try:
                date_obj = datetime.strptime(date_str, "%d.%m.%Y")
                formatted_date_str = date_obj.strftime("%d.%m.%Y")  # Обеспечиваем единый формат
                self.log(f"Дата корректна и отформатирована: {formatted_date_str}")
            except ValueError:
                self.log(f"Предупреждение: Дата {date_str} имеет неправильный формат. Пытаемся использовать как есть.")
            ...
            # Устанавливаем дату
            helper.set_date(formatted_date_str)
            
            # Получаем список контролеров из файла control.xlsx
            try:
                controllers = helper.get_controllers_from_excel("control.xlsx", formatted_date_str)
                if controllers:
            ...
            success = helper.load_from_control_xlsx("control.xlsx", formatted_date_str)
            if not success:
